...
export default function Conversion({children}){
        ...
        (async () => {
            ...
            while (!exchangeRate){
                try {
                    ...
                    if (json.Valute && json.Valute.USD) {
                        exchangeRate = json.Valute.USD.Value;
                        setExchangeRate(exchangeRate);
                    }
                } catch (error) {
                    date.setDate(date.getDate() - 1);
                    console.log(date.getDate())
                    console.log('отматали на день')
                }
            }
            // const response = await fetch(`https://www.cbr-xml-daily.ru/archive/${date.getFullYear()}/${date.getMonth()+1}/${date.getDate()}/daily_json.js`);
            // const json = await response.json();
    ...
    const handleInputChange = (e) => {
        ...
        setDollars(formatNumber(numericValue));

        const dollarsValue = parseFloat(numericValue);
        if (!isNaN(dollarsValue)) {
            const rublesValue = (dollarsValue * exchangeRate).toFixed(2);
            setRubles(formatNumber(rublesValue));
        } else {
            setRubles("");
        }
    };
    ...
    const handleSwitch = () => {
        if (!dollars && !rubles) return;

        if (dollars) {
            const rublesValue = parseFloat(dollars.replace(/\s/g, "").replace(",", ".")) * exchangeRate;
            setRubles(formatNumber(rublesValue.toFixed(2)));
            setDollars("");
        } else if (rubles) {
            const dollarsValue = parseFloat(rubles.replace(/\s/g, "").replace(",", ".")) / exchangeRate;
            setDollars(formatNumber(dollarsValue.toFixed(2)));
            setRubles("");
        }
    };
                ...
                <div className="temp">
                    ...
                    <div className="output_currency">
                        ...
                        <input
                            ...
                            value={rubles}
                            readOnly
                            placeholder={exchangeRate ? exchangeRate.toFixed(2) : ""}
                        />
