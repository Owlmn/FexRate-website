...
export default function Conversion({children}){
    ...
    useEffect(() => {
            (async () => {
                ...
                while (!rates){
                    try {
                        const response = await fetch(`https://www.cbr-xml-daily.ru/archive/${date.getFullYear()}/${date.getMonth()+1}/${day}/daily_json.js`);
                        if (!response.ok) {
                        ...
                        if (json.Valute) {
                            rates = {
                                RUB: 1,
                                USD: 1 / json.Valute.USD.Value,
                            };
                            Object.entries(json.Valute).forEach(([code, { Value, Nominal }]) => {
                                if (code !== "RUB") {
    ...
    const calculateConversion = (amount, fromCurrency, toCurrency) => {
        ...
        if (!isNaN(fromValue)) {
            ...
            const convertedAmount = (fromValue * fromRate / toRate).toFixed(4);
    ...
    const handleInputChange = (e) => {
        ...
        setToAmount(calculateConversion(numericValue, selectedFromCurrency, selectedToCurrency));
    };
    ...
    const handleSwitch = () => {
        ...
        if (fromAmount) {
            setToAmount(formatNumber(fromAmount));
            setFromAmount("");
        } else if (toAmount) {
            setFromAmount(formatNumber(toAmount));
            setToAmount("");
        }
    };
    ...
    useEffect(() => {
        if (fromAmount) {
            setToAmount(calculateConversion(fromAmount, selectedFromCurrency, selectedToCurrency));
        }
            ...
            <div className="convert">
                <div className="temp">
                    ...
                    <div className="exchange_method">
                        ...
                        <button
                            className={`currency_button ${activeButton === "Валюта" ? "active" : ""}`}
                            onClick={() => handleButtonClick("Валюта")}
                        >
                            <div className="conversion_text">Валюта</div>
                        </button>
                        <button
                            className={`currency_button ${activeButton === "Криптовалюта" ? "active" : ""}`}
                            onClick={() => handleButtonClick("Криптовалюта")}
                        >
                            <div className="conversion_text">Криптовалюта</div>
                        </button>
                        <button
                            className={`currency_button ${activeButton === "Акции" ? "active" : ""}`}
                    ...
                    <div className="input_currency">
                        ...
                        >
                            {Object.entries(currencies).map(([code, {icon, name, ruName}]) => (
                                <option key={code} value={code}>
                                    <span className="currency-option">
                                        <Image src={icon} alt=""/> {ruName} {name}
                                    </span>
                                </option>
                            ))}
                        </select>
                    ...
                    <div className="output_currency">
                        ...
                        <input
                            ...
                            value={toAmount}
                            readOnly
                            placeholder={exchangeRates[selectedToCurrency] ?
                                (exchangeRates[selectedFromCurrency] / exchangeRates[selectedToCurrency]).toFixed(4) : ""}
                        />
                        <select
